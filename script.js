// TODO: I hate CSS, should just use canvas instead...
...
function FormatClues(){
  ...
  for(i = 0; i < wordElements.length; i++){
    var lineEle = cluesAcross.appendChild(wordElements[i].ele),
        numEle = lineEle.getElementsByClassName("lineNum")[0];
        numEle.innerHTML = wordElements[i].num;  
  ...
  if (c.value !== null){
    ...
    for(var i=0 ; i < c.char.length; i++){
      ...
      if (c.char[i].prev === null){
        ...
        if (num === ""){
            num = wordElements.length + 1;
        }    
          wordElements.push({num:num,ele:matchingObj.element});
